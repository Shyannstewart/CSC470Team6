...
class OOAD:
    ...
    def stay_valid(self, s):
        return str(s).isdigit() and 1 <= int(s) <= 14
...
class Reports:
    ...
    def expected_occupancy_report(self):
        ...
        for i in range(days):
            ...
            occupancy_rate = (total / self.capacity) * 100 if self.capacity else 0
        ...
        avg_rate = total_occupancy_percent / days if days else 0
...
def demo_ooad_and_reports():
    ...
    print(report.expected_occupancy_report()[:200])  # print first part for demo
...
def run_gui():
    ...
    def open_payment_window():
        ...
        pay.title(f"Payment Info — Reservation #{res_id}")
        pay.geometry("350x300")

        tk.Label(pay, text="Card Number:").grid(row=0, column=0, pady=5, sticky="w")
        pan_entry = tk.Entry(pay, width=25)
        pan_entry.grid(row=0, column=1, pady=5)

        tk.Label(pay, text="Exp Month (MM):").grid(row=1, column=0, pady=5, sticky="w")
        exp_m_entry = tk.Entry(pay, width=5)
        exp_m_entry.grid(row=1, column=1, pady=5, sticky="w")

        tk.Label(pay, text="Exp Year (YY):").grid(row=2, column=0, pady=5, sticky="w")
        exp_y_entry = tk.Entry(pay, width=5)
        exp_y_entry.grid(row=2, column=1, pady=5, sticky="w")

        tk.Label(pay, text="Cardholder Name:").grid(row=3, column=0, pady=5, sticky="w")
        holder_entry = tk.Entry(pay, width=25)
        holder_entry.grid(row=3, column=1, pady=5)

        tk.Label(pay, text="CVV:").grid(row=4, column=0, pady=5, sticky="w")
        cvv_entry = tk.Entry(pay, width=6, show="*")
        cvv_entry.grid(row=4, column=1, pady=5, sticky="w")

        def submit_payment():
            pan = pan_entry.get().strip()
            mm = exp_m_entry.get().strip()
            yy = exp_y_entry.get().strip()
            nm = holder_entry.get().strip()
            cvv = cvv_entry.get().strip()

            try:
                rec = store_card_info(pan, int(mm), int(yy), nm, cvv)
                token = rec["token"]
                res.card_token = token
                res.paid_date = date.today()

                disp = get_card_display(token)
                msg = (
                    f"Card Stored Successfully\n"
                    f"{disp['card_type']} ending in {disp['last4']}\n"
                    f"Exp: {disp['exp_month']}/{disp['exp_year']}"
                )
        ...
        submit_button = tk.Button(pay, text="Save Payment Info", command=submit_payment)
    ...
    income_button.grid(row=3, column=0, columnspan=3, pady=10)

    def show_expected_occupancy():
        report = Reports(ooad)
        text = report.expected_occupancy_report()

        win = tk.Toplevel(root)
        win.title("Expected Occupancy Report (30 days)")
    ...
    occ_button = tk.Button(list_frame, text="Show Expected Occupancy", command=show_expected_occupancy)
    occ_button.grid(row=4, column=0, columnspan=3, pady=10)
    ...
    def check_availability():
        ...
        def do_check():
            ...
            report_lines.append("-" * 40 + "\n")
            # compute min_available safely
            min_available = min(
                capacity - sum(
                    1 for res in ooad.reservations.values()
                    if res.status in ["Active", "CHECKED IN"]
                    and res.arrival <= (check_in_date + timedelta(days=i)) < (res.arrival + timedelta(days=res.days))
                ) for i in range(nights)
            )
    ...
    availability_button.grid(row=9, column=0, columnspan=3, pady=5)
    payment_button = tk.Button(list_frame, text="Add Card", command=open_payment_window)
    payment_button.grid(row=10, column=0, columnspan=3, pady=10)
...
def is_room_available_for_dates(room_number, check_in_date, check_out_date, rooms_data):
    ...
    for reservation in room["reservations"]:
        res_check_in = dt.datetime.strptime(reservation["check_in_date"], "%Y-%m-%d")
        res_check_out = dt.datetime.strptime(reservation["check_out_date"], "%Y-%m-%d")
...
def verify_room_availability(rooms_data):
    ...
    try:
        check_in_date = dt.datetime.strptime(check_in_str, "%Y-%m-%d")
        check_out_date = dt.datetime.strptime(check_out_str, "%Y-%m-%d")
    except ValueError:
...
def check_in_guest(rooms_data):
    ...
    for reservation in room["reservations"]:
        if reservation["guest_name"].lower() == guest_name.lower() and not reservation.get("checked_in", False):
    ...
    log_action("CHECK_IN", room_number=room_number, guest_name=guest_name,
               extra_info=f"Check-out: {reservation_found.get('check_out_date')}")
...
def check_out_guest(rooms_data):
    ...
    guest_name = room.get("current_guest")

    total_bill = 0
    for i, reservation in enumerate(list(room["reservations"])):
        if reservation.get("guest_name", "").lower() == (guest_name or "").lower() and reservation.get("checked_in", False):
            check_in = dt.datetime.strptime(reservation["check_in_date"], "%Y-%m-%d")
            check_out = dt.datetime.strptime(reservation["check_out_date"], "%Y-%m-%d")
